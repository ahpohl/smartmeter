...
data = from(bucket: "smartmeter-live")
  |> range(start: -28h)
  |> filter(fn: (r) => r._measurement == "state" and (r._field == "energy" or r._field == "rate" or r._field == "price"))
  |> drop(columns: ["custom_id","device_id","serial","status"])
  |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")
day_start = data |> aggregateWindow(every: 1d, fn: first, column: "energy")
day_end = data |> aggregateWindow(every: 1d, fn: last, column: "energy")
res = join(tables: {t1: day_start, t2: day_end}, on: ["_time", "_measurement", "_start", "_stop", "price", "rate"])
energy = res |> map(fn: (r) => ({
    _time: r._time,
...
total = res |> drop(columns: ["energy_t1", "price", "rate"])
