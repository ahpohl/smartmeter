...
void SmartmeterMqtt::SetDebug(const bool &debug) {
  Log = debug;
}
...
bool SmartmeterMqtt::SetUserPassAuth(const std::string &user,
  ...
  if ((rc = mosquitto_username_pw_set(Mosq, user.c_str(), pass.c_str()))) {
    ErrorMessage = std::string(
            "Mosquitto unable to enable password authentication: ")
...
bool SmartmeterMqtt::Connect(const std::string &host,
                             const int &port,
                             const int &keepalive) {
  int rc = 0;
  if ((rc = mosquitto_loop_start(Mosq))) {
    ErrorMessage = std::string("Mosquitto loop start failed: ")
            + mosquitto_strerror(rc);
    return false;
  }
  if ((rc = mosquitto_connect_async(Mosq, host.c_str(), port, keepalive))) {
    ErrorMessage = std::string("Mosquitto unable to connect: ")
            + mosquitto_strerror(rc);
    return false;
  }
...
bool SmartmeterMqtt::PublishMessage(const std::string &message,
  ...
  if ((rc = mosquitto_publish(Mosq,
                             ...
                             qos,
                             retain))) {
    ErrorMessage = std::string("Mosquitto publish failed: ")
        + mosquitto_strerror(rc);
    IsConnected = false;
    return false;
  }
...
void SmartmeterMqtt::OnConnectCallback(struct mosquitto *mosq,
                                       void *obj,
                                       int connack_code) {
  if (!connack_code) {
    IsConnected = true;
...
void SmartmeterMqtt::OnConnectCallbackWrapper(struct mosquitto *mosq,
                                              void *obj,
                                              int connack_code) {
...
void SmartmeterMqtt::LogCallback(struct mosquitto *mosq,
                                 void *obj,
                                 int level,
                                 const char *str) {
  if (Log) {
...
void SmartmeterMqtt::LogCallbackWrapper(struct mosquitto *mosq,
                                        void *obj,
                                        int level,
                                        const char *str) {
  auto *p = reinterpret_cast<SmartmeterMqtt*>(obj);
